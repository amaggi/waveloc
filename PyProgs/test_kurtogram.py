...
class KurtogramTests(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.grid, self.level_w, self.freq_w, self.c, self.flow, self.fup = Fast_Kurtogram(self.x, self.nlevel, Fs=self.Fs)
  ...
  def test_BandwidthFrequency(self):
    ...
    bw, fc, fi, l1 = getBandwidthAndFrequency(self.nlevel,self.Fs, self.level_w,
  ...
  def test_FindWavKurt(self):
    ...
    bw_hz, fc_hz, fi, l1 = getBandwidthAndFrequency(self.nlevel, self.Fs,
