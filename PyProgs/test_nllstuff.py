...
class ProjectionTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.proj_info = {}
        self.proj_info['orig_lat'] = self.true_lat + \
            np.random.ranf()*10.0 - 5.0
        self.proj_info['orig_lon'] = self.true_lon + \
            np.random.ranf()*10.0 - 5.0
    ...
    def test_TransUnknown(self):
        ...
        self.assertRaises(UserWarning, latlon2rect, 'TRANS_BUG', self.true_lat,
                          self.true_lon)
        self.assertRaises(UserWarning, rect2latlon, 'TRANS_BUG', self.true_lat,
                          self.true_lon)

    def test_TransSimple(self):

        x, y = latlon2rect('TRANS_SIMPLE', self.true_lat, self.true_lon,
                           self.proj_info)
