...
def setUpModule():
    ...
    test_files = ['test_grid.search.hdr', 'coord_stations_test',
                  'grid.Taisne.search.hdr', 'test_channel_file']
    ...
    test_files = glob.glob(os.path.join(base_path, 'test_data', 'time_grids',
                                        'Slow*'))
    if test_files == []:
        logging.error('Dowload\
            https://github.com/downloads/amaggi/waveloc/test_data.tgz\
            and unpack it in the %s directory, then re-run' % (base_path))
    for tfile in test_files:
        try:
