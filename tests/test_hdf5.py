...
import unittest
import numpy as np
from waveloc.hdf5_grids import H5SingleGrid, H5NllSingleGrid, nll2hdf5
...
class H5Tests(unittest.TestCase):
    ...
    def test_MemoryPerformance(self):
        ...
        for i in range(n_slices):
            imax[i*t_slice:(i+1)*t_slice] = \
                np.argmax(bigdata[:, i*t_slice:(i+1)*t_slice], 0)
            maxdata[i*t_slice:(i+1)*t_slice] = \
                np.max(bigdata[:, i*t_slice:(i+1)*t_slice], 0)
        imax[n_slices*t_slice:nt] = \
            np.argmax(bigdata[:, n_slices*t_slice:nt], 0)
        maxdata[n_slices*t_slice:nt] = \
            np.max(bigdata[:, n_slices*t_slice:nt], 0)
...
class H5SingleGridTests(unittest.TestCase):
    ...
    def test_NllRead(self):
        ...
        b_index = np.random.randint(0, nx) * np.random.randint(0, ny) * \
